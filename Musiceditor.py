...
root = tk.ThemedTk(theme="radiance")
root["bg"]="#18cca8"

#Establishing the visual theme for the player

root.set_theme("radiance")
statusbar = ttk.Label(root, text="Welcome to Melox Music Player", anchor=W, font='Montserrat 8 bold')
statusbar.pack(side=BOTTOM, fill=X)
statusbar1 = ttk.Label(root, text="Welcome to Melox Music Player", anchor=W, font='Montserrat 8 bold')
...
#List for storing playback information.
...
root.title("Melox Music Player")
ltframe = Frame(root,bg="#f0553a")
ltframe.pack(side=LEFT, padx=30, pady=30)
songplaylistcontainer = Listbox(ltframe,fg='white')
songplaylistcontainer["bg"]="#f0553a"
...
root.style.configure('TButton', background='#15cdd6')
rtframe = Frame(root,bg="#f7e843")
rtframe.pack(pady=30,padx=20)
topframe = Frame(rtframe,bg="#f7e843")
topframe.pack()
root.style = ttk.Style()
...
def play_music():
    ...
    if paused:
        ...
        try:
            ...
            tkinter.messagebox.showerror('File not found', 'Melox Music player could not find the file. Please select again.')
def stop_music():
    mixer.music.stop()
...
scale.set(70)  #Set the default value for the scale when the music player starts.
